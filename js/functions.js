jQuery(function(){
	...
	// PEGA A POSIÇÂO DO SCROLL (inutilizado)
	...
	function lidos(){
		jQuery.each(jQuery('.window'), function(index){
			...
			var li = jQuery('#con_'+idConversa+' #12.eu');

			var lastLi = jQuery('#con_'+idConversa+ ' li');

			if (jQuery(lastLi.last()).hasClass("eu")) {
				jQuery.ajax({
					type: 'POST',
					url: 'sys/ler.php',
					data: {lido: 'sim', online: userOnline, user: idConversa},
					success: function(retorno){
						if (retorno == 1) {
							lastLi.last().after('<li id="lido">visualizado</li>');
							jQuery('#janela_' + idConversa + ' .mensagens').scrollTop(250);		
						}							
					}
				});
			}
	...
	function add_janela(id, nome, status){
		...
		janela += '<div class="body"><div class="mensagens"><ul id="con_'+id_user+'"></ul></div>';
	...
	jQuery('body').on('click', '.close', function(){
		...
		for (var i = 1; i <= restamAfrente; i++) {
			jQuery('.window:eq(' + (indice+i) + ')').animate({right:"-=275"}, 200);
		}
		parent.remove();
	...
	jQuery('body').on('click', '.mensagens', function(){
		...
		jQuery.ajax({
			...
			success: function(retorno){
				//lidos(); so funciona o lidos quando lido nao roda
				lidos();
			}
		});
	});
